...
public class PrintPreview extends JScrollPane {
   ...
   private static final long serialVersionUID = 1L;
    ...
    private void setupControls(){
            ...
            mPrevButton.addActionListener((ActionEvent ae) -> {
                changePage(mCurrentPage-1);
            });

            mNextButton.addActionListener((ActionEvent e) -> {
                changePage(mCurrentPage+1);
            });

            mPagesCombo.addItemListener((ItemEvent arg0) -> {
                changePage(Integer.parseInt((String)mPagesCombo.getSelectedItem()));
            });

            mZoomSlider.addChangeListener((ChangeEvent arg0) -> {
    ...
    */
    public void print(){
        try {
                ...
                if (pj.printDialog())
                        pj.print();
        } catch (HeadlessException | PrinterException | IndexOutOfBoundsException | NullPointerException ex) {
    ...
    public void printCurrentPage() {
        try {
                ...
                if (pj.printDialog(pra))
                        pj.print(pra);
        } catch (HeadlessException | PrinterException | IndexOutOfBoundsException ex) {
                JOptionPane.showMessageDialog(null, ex.toString(),
                                "Error in Printing", 1);
        }
    }
